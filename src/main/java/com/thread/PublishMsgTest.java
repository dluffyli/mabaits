...
public class PublishMsgTest {
    ...
    //发送消息的业务逻辑方法
    public int sendMsgs(List<Integer> receivers, final String content) {
        long begin = System.nanoTime();
        AtomicInteger ai = new AtomicInteger(0);
        List<Future<Integer>> list = new ArrayList<>();
        //循环发送消息
        for (int i = 0; i < receivers.size(); i++) {
            final Integer receiver = receivers.get(i);
    ...
    public static void main(String[] args) {
        ...
        int successCount = pmt.sendMsgs(receivers, content);
        System.out.println("共有【" + receivers.size() + "】接收者，发送成功【" + successCount + "】");
    }

    //完成发送消息
    public int sendMsg(Integer receiver, String content) {
